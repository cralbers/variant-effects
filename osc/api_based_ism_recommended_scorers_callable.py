...
# ISM_START = 155_611_327    
# ISM_END   = 156_611_327

ISM_START = int(sys.argv[1])
ISM_END = int(sys.argv[2])
...
SCORES_CSV = str(Path(__file__).parent / f"lmna_ism_out/lmna_ism_{ISM_START}_{ISM_END}_{SEQ_LENGTH}.csv")
bed_path = 'data_sync/predicted_ccre_like_regions_central_LV_128res.bed'
...
def _build_scorers():
    ...
    scorers = []
    for key, scorer in variant_scorers.RECOMMENDED_VARIANT_SCORERS.items():
        scorers.append((f"{key}, RECOMMENDED", key, scorer))
...
def run_one_piece(api_key, chunk_start, chunk_end, out_queue):
    ...
    print(f"  [worker] start {CHROM}:{chunk_start}-{chunk_end}  "
          f"ism={ism_interval}  seq={seq_interval}  "
          f"n_scorers={len(scorers)}", flush=True)
    ...
    try:
        ...
        print(f"  [worker] ok {CHROM}:{chunk_start}-{chunk_end}  "
              f"{len(partial)} variants  {elapsed:.1f}s", flush=True)
        out_queue.put({"status": "ok", "elapsed": elapsed, "partial": partial})
    except Exception as e:
        tb = traceback.format_exc()
        print(f"  [worker] ERROR {CHROM}:{chunk_start}-{chunk_end}:\n{tb}", flush=True)
        out_queue.put({
            "status": "error",
            "elapsed": time.time() - t0,
            "partial": {},
...
def key_dispatcher(worker_id, api_key, my_chunks, rows_by_position,
    ...
    for chunk_start, chunk_end in my_chunks:
        print(f"  [key {worker_id}] starting {CHROM}:{chunk_start}-{chunk_end} "
              f"(waiting up to {CALL_TIMEOUT_S}s)...", flush=True)
        out_queue = ctx.Queue()
        ...
        if status == "ok":
            status_msg = f"ok  {result['elapsed']:.1f}s"
        elif status == "timeout":
            status_msg = f"TIMEOUT after {CALL_TIMEOUT_S}s"
        else:
            err = result.get("error", "unknown error")
            status_msg = f"ERROR -- {err}"
            tb = result.get("traceback")
            if tb:
                print(f"  [key {worker_id}] traceback for "
...
def main() -> None:
    ...
    print("=" * 60, flush=True)
    print("ISM debug config:", flush=True)
    print(f"  API_KEYS         = {len(API_KEYS)}", flush=True)
    print(f"  CHROM / ISM      = {CHROM}:{ISM_START}-{ISM_END}", flush=True)
    print(f"  CHUNK_BP         = {CHUNK_BP}", flush=True)
    print(f"  SEQ_LENGTH       = {SEQ_LENGTH}", flush=True)
    print(f"  CALL_TIMEOUT_S   = {CALL_TIMEOUT_S}", flush=True)
    print(f"  MAX_WORKERS      = {MAX_WORKERS}", flush=True)
    print(f"  TISSUE_ONTOLOGY  = {TISSUE_ONTOLOGY}", flush=True)
    print(f"  SCORES_CSV       = {SCORES_CSV}", flush=True)
    print(f"  scorers ({len(score_cols)}): {score_cols}", flush=True)
    print("=" * 60, flush=True)
    ...
    if not Path(SCORES_CSV).exists():
        ...
        print(f"Pre-initialised {SCORES_CSV} with {len(rows_list):,} rows "
              f"({len(ref_seq):,} positions x 3 alts).", flush=True)
    else:
        ...
        print(f"Loaded existing {SCORES_CSV} ({len(rows_list):,} rows).", flush=True)
    ...
    print(f"Launching {len(API_KEYS)} key-dispatchers over {len(todo)} chunks "
          f"({CHUNK_BP} bp each)...", flush=True)
    for i in range(len(API_KEYS)):
        print(f"  key {i + 1}: {len(key_chunks[i])} chunks", flush=True)
    print(flush=True)
    run_start = time.time()
    ...
    print(f"Finished in {elapsed_min:.1f} min | {counters['ok']} chunks ok | "
          f"{counters['failed']} chunks left blank", flush=True)
    print("Re-run the script to retry the blank cells." if counters["failed"]
          else "All chunks complete.", flush=True)
